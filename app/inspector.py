...
from app.utils import align_images, save_result_image, draw_text_bg
...
class PCBInspector:
    ...
    def _load_models(self):
        ...
        if list(ckpt.keys())[0].startswith('module.'): ckpt = {k[7:]: v for k, v in ckpt.items()}
        ...
        if list(emb_state_dict.keys())[0].startswith('module.'): emb_state_dict = {k[7:]: v for k, v in
                                                                                   emb_state_dict.items()}
    ...
    def _build_response(self, test, gold, heat, over, final, dets):
        ...
        return {
            ...
            "images": {
                ...
                "template": save_result_image(gold, "gold"),
                "heatmap": save_result_image(heat, "heat"),
                "mask_overlay": save_result_image(over, "mask"),
                "final": save_result_image(final, "final")
            }
        }